...
export default function QuizList() {
   ...
   return (
      <Fragment>
         ...
         {quizzesApi.isLoading ? <PageLoader /> : null}
         <section className='py-6'>
            <div className='mx-auto max-w-7xl px-5 lg:px-0'>
               <h1 className='text-2xl font-bold mt-4'>All Quiz List</h1>
               <div className='mt-4 bg-opacity-10'>
                  <div className='w-full flex'>
                     <Link to='/admin/quiz/add' className='btn ml-auto'>
                        Add Quiz
                     </Link>
                  </div>
                  <div className='overflow-x-auto mt-4'>
                     {quizzesApi.data?.length ? (
                        <table className='divide-y-1 text-base divide-gray-600 w-full'>
                           ...
                           <tbody className='divide-y divide-slate-600/50'>
                              {quizzesApi.data?.map((item, index) => (
                                 <tr key={`quiz-${index}`} className='video-table-row'>
                                    <td className='table-td'>{item.question}</td>
                                    <td className='table-td'>{item.video_title}</td>
                                    <td className='table-td flex gap-x-2 justify-end'>
                                       <svg
                                          fill='none'
                                          viewBox='0 0 24 24'
                                          strokeWidth='1.5'
                                       ...
                                       <Link to={`/admin/quiz/edit/${item.id}_${stringToUrl(item.question)}`}>
                                          <svg
                                             fill='none'
                                             viewBox='0 0 24 24'
                                             strokeWidth='1.5'
                        ...
                        <Empty scope='inner' text='No quizzes found to show here!' />
                     )}
                  </div>
               </div>
            </div>
